...
* *Root theme transition* - View Transitions when available (`html.adt-theme-vt` suppresses competing CSS paint transitions); otherwise a short `html.adt-theme-animating` class so paint properties crossfade site-wide (skipped under `prefers-reduced-motion: reduce`)
...
function runThemePaint(updateFn, animate) {
  ...
  if (typeof document.startViewTransition === 'function') {
    html.classList.add('adt-theme-vt')
    document.startViewTransition(updateFn).finished.finally(() => {
      html.classList.remove('adt-theme-vt')
    })
    return
  }
