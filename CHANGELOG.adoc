...
* *Fix:* Admonition card tints set only `background` (drop solid `background-color` fallbacks that could two-tone the TIP card against `color-mix`).
...
* *Fix:* Theme toggle timing — during View Transitions, set `html.adt-theme-vt` and match `:active-view-transition` with `transition: none` so Valentus/search per-widget paint transitions (0.2s) cannot finish ahead of the 0.28s root snapshot crossfade. See link:changelog-details/2026-08-08%20-%20theme%20transition%20vt%20suppress.adoc[Detailed notes].

== [1.4.1] - 2026-08-08

* *Fix:* Dark-mode admonitions — stop painting `td.icon` / table chrome with type tints; TIP/NOTE labels sit above the bordered content card without a green (etc.) wash. See link:changelog-details/2026-08-08%20-%20admonition%20dark-mode%20label%20wash.adoc[Detailed notes].
...
* *Documentation:* Version Pinning guide (exact pin, `releases/latest`, Renovate `~1.0.0`, rolling line URLs); `examples/renovate.json` regex manager for playbook URLs.
* *Semver:* Minor line for additive branding and pinning infrastructure (`site.keys.header_logo` shipped in v1.0.12).

link:changelog-details/2026-06-08%20-%201.1.0%20version%20pinning%20and%20rolling%20lines.adoc[Detailed notes — 2026-06-08]

== [1.0.12] - 2026-06-08

* *Branding:* `header-content.hbs` — playbook `site.keys.header_logo` selects the header image basename under `img/` (default `logo.svg`); consumers can brand via supplemental assets without forking the header partial.

link:changelog-details/2026-06-08%20-%201.0.12%20header%20logo%20site%20key.adoc[Detailed notes — 2026-06-08]

== [1.0.11] - 2026-06-08

* *Bugfix:* `site-dark-mode.js` — retry VCS icon loads with `#site-script` `data-ui-root-path` when the first asset base 404s (local preview).

link:changelog-details/2026-06-08%20-%201.0.11%20VCS%20icon%20path%20retry.adoc[Detailed notes — 2026-06-08]

== [1.0.10] - 2026-06-08

* *Theme:* `site-dark-mode.js` — user light/dark override expires when the OS color scheme changes (`antora-theme-system-snapshot`, `matchMedia` listener, tab focus/visibility recheck).
* *Search:* Demo playbooks register `@antora/lunr-extension`; supplemental header exposes the search field; dark-mode styles for Lunr results dropdown.
* *npm registry:* Formal deprecation notice in `README.md` (npmjs package page) and `deprecated` field in `package.json`; maintainer steps in `docs/maintainers/npm-registry-notice.md`.
* *Distribution:* Remove npm publish from link:.github/workflows/release.yml[release workflow]; GitHub Releases ship `ui-bundle.zip` only. `package.json` marked `private` (repo tooling, not a registry package).
* *Documentation:* npm install/migration detail in `docs/modules/guide/pages/npm-deprecation.adoc`; Extensions and Configuration pages document Lunr search.

link:changelog-details/2026-06-08%20-%201.0.10%20system%20theme%20and%20lunr%20search.adoc[Detailed notes — 2026-06-08]

== [1.0.9] - 2026-04-26

* *UI:* `site-doc-layout.css` — double horizontal rail padding (`--adt-mast-pad-h` **0.5rem → 1rem**) so the left nav tree, doc mast, and breadcrumb home share a roomier inset.

link:changelog-details/2026-04-26%20-%201.0.9%20nav%20rail%20horizontal%20padding.adoc[Detailed notes — 2026-04-26]

== [1.0.8] - 2026-04-26

* *UI:* `site-doc-layout.css` — placeholder `img/logo.svg` uses `currentColor`, which does not apply when the SVG is loaded as a raster `<img>`, so the glyph read as dark on the dark header. In `html.dark-theme`, apply `filter: brightness(0) invert(1)` (and slight opacity) so the logo matches the light top-bar foreground.

link:changelog-details/2026-04-26%20-%201.0.8%20header%20logo%20dark%20mode.adoc[Detailed notes — 2026-04-26]
